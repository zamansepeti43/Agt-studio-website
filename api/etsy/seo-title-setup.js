...
const SEO_UPDATES = {
  4574114283: {
    ...
    tags: ['bakery pricing tool','recipe cost sheet','recipe costing','home bakery pricing','home bakery business','bakery profit calc','food cost calculator','cake pricing','cake costing','order tracker','bakery spreadsheet','baking business','bakery cost tracker'],
    descriptionIntro: 'Bakery Pricing Calculator Excel Spreadsheet for home bakers, cake makers and small bakeries. Calculate recipe cost, food cost, selling price, profit margin and track orders in one workbook.',
  },
  4575285227: {
    title: 'Etsy Seller Customer Service Templates | 120 Buyer Reply Scripts | ChatGPT Prompts | Excel',
    tags: ['etsy seller support','etsy support tool','etsy customer reply','etsy reply templates','customer service','reply templates','buyer replies','ai reply generator','chatgpt prompts','refund response','review response','etsy message replies','excel template'],
    descriptionIntro: 'Etsy Seller Customer Service Templates with 120 buyer reply scenarios, ChatGPT prompts and an Excel-based workflow for handling common Etsy customer messages.',
  },
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const listing of live?.results || []) {
      ...
      changed.push({ listing_id: id, old_title: listing.title, new_title: update.title, tags_updated: tagsChanged, description_updated: descriptionChanged });
    }
    ...
    res.status(200).json({
      ...
      changed,
      skipped,
      message: `${changed.length} hedef Etsy ilanında SEO başlığı, etiket ve açıklama güncellemesi uygulandı. Fiyat ve görseller değiştirilmedi.`,
    });
  } catch (error) {
