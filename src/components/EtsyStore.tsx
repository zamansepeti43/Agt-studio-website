...
export default function EtsyStore() {
  ...
  useEffect(() => {
    ...
    fetch("/api/etsy/listings?state=active&limit=50&offset=0")
      .then((response) => {
        if (!response.ok) throw new Error("Etsy products unavailable");
        return response.json() as Promise<EtsyResponse>;
      })
      .then((data) => {
        if (cancelled) return;
        setListings(data.listings?.results || []);
  ...
  return (
    <section id="etsy" className="etsy-store-section">
      ...
      {loading ? (
        <div className="etsy-store-state">{t("loadingProducts")}</div>
      ) : listings.length === 0 ? (
        <div className="etsy-store-state">
          <div>
            <p>{t("productsUnavailable")}</p>
        ...
        <>
          <div className="etsy-products-grid">
            {listings.map((listing) => {
              ...
              const url =
                listing.url || "https://www.etsy.com/listing/" + listing.listing_id;
              const title =
                (language === "tr" ? turkishTitles : englishTitles)[
                  listing.listing_id
                ] || listing.title;
                    ...
                    {image ? (
                      <img
                        ...
                        onError={(event) => {
                          ...
                          event.currentTarget.parentElement?.classList.add("etsy-image-failed");
                        }}
                      />
                    ) : (
            ...
            <a
              className="etsy-product-more-card"
              href={shopUrl}
              target="_blank"
              rel="noopener noreferrer"
            >
              <span className="etsy-more-plus">＋</span>
              <strong>
                {language === "tr" ? "Daha fazla ürün keşfet" : "Discover more products"}
              </strong>
              <span className="etsy-more-arrow">→</span>
            </a>
          </div>
